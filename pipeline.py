...
def run_experiment(cfg):
    ...
    # Data Preprocessing
    preprocess_dict = config['artifact_paths']['cleaning']
    ...
    # Featurization
    featurize_dict = config['featurize']
    if featurize_dict['train_test_split']['bool'] == True:
        ...
        config['featurize']['splits'] = splits
    ...
    X_train, X_val, X_test  = feature_scaling(X_train, X_val, X_test, 
                                                scaler_type=featurize_dict['scaling']['type'])
    
    # Training
    train_dict = config['train']
    if train_dict['bool'] == True:
        ...
        for tup in crop_map_dict.items():
            pickling(task='write', object=tup[1][0], path=model_dir_path+tup[0]+'.pkl')
    else: 
        pass
    
    # Evaluation
    eval_dict = config['inference']
    if eval_dict['bool'] == True:
        test_reports, tar_vs_fns, conf_mat_plots, roc_curves, pr_curves = rabi_models_testing(X_test, y_test,
                                                                                            crop_map_dict,
